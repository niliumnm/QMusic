...
void MainWindow::onDrawTune(QString tuneStr)
{
	...
	//scene->setSceneRect(0, 0, 300, 300);
	if (tunePicItem) {
		scene->removeItem(tunePicItem);
	}

	QPixmap notePic = QPixmap(path);
	tunePicItem = new QGraphicsPixmapItem(notePic);
	tunePicItem->setScale(1.5);
	tunePicItem->setPos(20, 20);
	scene->addItem(tunePicItem);
	qDebug() << ui->noteGraphView->scene();
	ui->noteGraphView->setScene(scene);
	qDebug() << ui->noteGraphView->scene();
	ui->noteGraphView->show();
...
inline bool MainWindow::isSame(QString strA, QString strB)
{
...
void MainWindow::on_tuneComb_currentIndexChanged(int index)
{
	...
	if (tune == QStringLiteral("C#大调|a#小调")) {
		noteShift = { +1,+1,+1,+1,+1,+1,+1 };
		emit drawTune("C#");
	}
	else if (tune == QStringLiteral("F#大调|d#小调")) {
		noteShift = { +1,+1,+1,+1,+1,+1,0 };
		emit drawTune("F#");
	}
	else if (tune == QStringLiteral("B大调|g#小调")) {
		noteShift = { +1,+1,0,+1,+1,+1,0 };
		emit drawTune("B");
	}
	else if (tune == QStringLiteral("E大调|c#小调")) {
		noteShift = { +1,+1,0,+1,+1,0,0 };
		emit drawTune("E");
	}
	else if (tune == QStringLiteral("A大调|f#小调")) {
		noteShift = { +1,0,0,+1,+1,0,0 };
		emit drawTune("A");
	}
	else if (tune == QStringLiteral("D大调|b小调")) {
		noteShift = { +1,0,0,+1,0,0,0 };
		emit drawTune("D");
	}
	else if (tune == QStringLiteral("G大调|e小调")) {
		noteShift = { 0,0,0,+1,0,0,0 };
		emit drawTune("G");
	}
	else if (tune == QStringLiteral("C大调")) {
		noteShift = { 0,0,0,0,0,0,0 };
		emit drawTune("C");
	}
	else if (tune == QStringLiteral("F大调|d小调")) {
		noteShift = { 0,0,0,0,0,0,-1 };
		emit drawTune("F");
	}
	else if (tune == QStringLiteral("Bb大调|g小调")) {
		noteShift = { 0,0,-1,0,0,0,-1 };
		emit drawTune("Bb");
	}
	else if (tune == QStringLiteral("Eb大调|c小调")) {
		noteShift = { 0,0,-1,0,0,-1,-1 };
		emit drawTune("Eb");
	}
	else if (tune == QStringLiteral("Ab大调|f小调")) {
		noteShift = { 0,-1,-1,0,0,-1,-1 };
		emit drawTune("Ab");
	}
	else if (tune == QStringLiteral("Db大调|bb小调")) {
		noteShift = { 0,-1,-1,0,-1,-1,-1 };
		emit drawTune("Db");
	}
	else if (tune == QStringLiteral("Gb大调|eb小调")) {
		noteShift = { -1,-1,-1,0,-1,-1,-1 };
		emit drawTune("Gb");
	}
	else if (tune == QStringLiteral("Cb大调|ab小调")) {
		noteShift = { -1,-1,-1,-1,-1,-1,-1 };
		emit drawTune("Cb");
	}
}
